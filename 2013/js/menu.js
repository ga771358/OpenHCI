$(function() {
    $("#head").css("background", "url(images/head.png) no-repeat");
    $("#btn1").css("background", "url(images/btn_01_on.png) no-repeat");
    var index = 1;
    init(1);
    ...
    $("a[id^=btn]").bind('mouseenter mouseleave', function(event) {
        ...
        switch (event.type) {
            case 'mouseenter':
                $(this).css("background", "url(images/btn_0" + id + "_on.png) no-repeat");
                break;
            case 'mouseleave':
                if (id != index) {
                    $(this).css("background", "url(images/btn_0" + id + ".png) no-repeat");
                }
                break;
        }
    });
...
function init(id) {
    ...
    $("a[id^=btn]").each(function(i, element) {
        var id = this.id.substring(3);
        $(element).css("background", "url(images/btn_0" + id + ".png) no-repeat");
    });

    $("#btn" + id).css("background", "url(images/btn_0"+id+"_on.png) no-repeat");
}